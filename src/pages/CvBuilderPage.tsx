...
function generatePdf(data: CvData): string {
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  const contact = [data.personal.email, data.personal.phone, data.personal.city]
    .filter(Boolean)
    .join("  •  ");
...
export default function CvBuilderPage() {
  ...
  const addLanguage = () => {
    ...
    if (l && !data.languages.includes(l))
      setData((p) => ({ ...p, languages: [...p.languages, l] }));
    setLangInput("");
  };
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () =>
      setData((p) => ({
        ...p,
        personal: { ...p.personal, photo: reader.result as string },
      }));
  ...
  const handleGenerate = async () => {
    if (!data.personal.firstName.trim() || !data.personal.email.trim()) {
      toast({
        title: "Missing info",
        description: "Please fill in at least your first name and email.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDownloadCvPdf = async () => {
    if (!user) {
      toast({
        title: "Error",
        description: "You must be logged in to download your CV.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to download CV from server.",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s.key} className="flex items-center">
                  ...
                  >
                    <div
                      className={`flex h-9 w-9 items-center justify-center rounded-full border-2 transition-colors ${active ? "border-primary-foreground bg-primary-foreground text-primary" : "border-primary-foreground/40 text-primary-foreground/40"}`}
                    >
                      <Icon className="h-4 w-4" />
                    </div>
                    <span
                      className={`text-xs font-medium ${active ? "text-primary-foreground" : "text-primary-foreground/40"}`}
                    >
                      {s.label}
                    </span>
                  </button>
                  {i < steps.length - 1 && (
                    <div
                      className={`mx-2 h-0.5 w-16 md:w-24 ${i < stepIndex ? "bg-primary-foreground" : "bg-primary-foreground/30"}`}
                    />
                  )}
                </div>
              );
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updatePersonal("postalCode", e.target.value)
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
          ...
          <>
            ...
            <ScrollReveal delay={40}>
              <Section
                ...
                defaultOpen={data.experience.length > 0}
              >
            ...
            <ScrollReveal delay={120}>
              <Section icon={Heart} title="Interests">
                <Textarea
                  ...
                  onChange={(e) =>
                    setData((p) => ({ ...p, interests: e.target.value }))
                  }
            ...
            <ScrollReveal delay={160}>
              <Section icon={MessageSquare} title="References">
                <Textarea
                  ...
                  onChange={(e) =>
                    setData((p) => ({ ...p, references: e.target.value }))
                  }
            ...
            <Card className="border shadow-sm">
              <CardContent className="p-5 md:p-8 space-y-6 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Review your details and generate your CV as a downloadable
                  PDF.
                </p>
