...
const CV_STORAGE_KEY = "trustbee_cv_draft";

/* ── Month/Year Picker ──────────────────────────────── */
const MONTHS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
function MonthYearPicker({ value, onChange, placeholder = "Select" }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  // value format: YYYY-MM
  const [y, m] = value ? value.split("-") : ["", ""];
  const currentYear = new Date().getFullYear();
  const years = Array.from({ length: 60 }, (_, i) => String(currentYear - i + 5));
  return (
...
export default function CvBuilderPage() {
  ...
  // Autosave: localStorage immediately, API debounced
  useEffect(() => {
    try {
    ...
    autosaveTimer.current = setTimeout(async () => {
      ...
      try {
        await api.updateJobSeeker(user.id, { cvDraft: JSON.stringify(data) } as any);
        setAutosaveStatus("saved");
      } catch {
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      {/* Content */}
      <div className="mx-auto max-w-3xl px-3 sm:px-4 py-6 sm:py-8 space-y-4">
        {/* Autosave indicator */}
        <div className="flex justify-end">
          <span className={`text-xs ${autosaveStatus === "offline" ? "text-destructive" : "text-muted-foreground"}`}>
            {autosaveStatus === "saving" && "Saving…"}
            {autosaveStatus === "saved" && "✓ Saved"}
            {autosaveStatus === "offline" && "⚠ Offline – saved locally"}
          </span>
        </div>
        {step === "personal" && (
          <ScrollReveal>
                  ...
                  {data.experience.map((exp) => (
                    <div key={exp.id} className="rounded-lg border p-4 space-y-3 relative">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">Start Date</Label><MonthYearPicker value={exp.startDate} onChange={(v) => updateExperience(exp.id, "startDate", v)} /></div>
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">End Date</Label><MonthYearPicker value={exp.endDate} onChange={(v) => updateExperience(exp.id, "endDate", v)} placeholder="Present" /></div>
                      </div>
                  ...
                  {data.education.map((edu) => (
                    <div key={edu.id} className="rounded-lg border p-4 space-y-3 relative">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">Start Date</Label><MonthYearPicker value={edu.startDate} onChange={(v) => updateEducation(edu.id, "startDate", v)} /></div>
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">End Date</Label><MonthYearPicker value={edu.endDate} onChange={(v) => updateEducation(edu.id, "endDate", v)} /></div>
                      </div>
