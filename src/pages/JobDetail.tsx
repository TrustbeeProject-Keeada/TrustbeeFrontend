...
import { MapPin, Briefcase, Calendar, ArrowLeft, Bookmark, Send, ExternalLink } from "lucide-react";
      ...
      <ScrollReveal delay={60}>
        <Card className="glass">
          <CardHeader>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                {job.company?.logoUrl ? (
                  <img src={job.company.logoUrl} alt={job.company.companyName} className="h-12 w-12 rounded-lg object-cover" />
                ) : (
                ...
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground">{job.company?.companyName}</p>
                </div>
              </div>
              {user?.role === "JOB_SEEKER" && (
                ...
                >
                  <Bookmark className={`h-5 w-5 ${isJobSaved(job.id) ? "fill-current" : ""}`} />
                </Button>
              )}
            </div>
            <div className="mt-3 flex flex-wrap gap-3 text-sm text-muted-foreground">
              {location && <span className="flex items-center gap-1"><MapPin className="h-4 w-4" />{location}</span>}
              {job.category && <span className="flex items-center gap-1"><Briefcase className="h-4 w-4" />{job.category}</span>}
              {job.expiresAt && <span className="flex items-center gap-1"><Calendar className="h-4 w-4" />Expires {new Date(job.expiresAt).toLocaleDateString()}</span>}
              {job.status && (
          ...
          <CardContent className="space-y-6">
            ...
            {job.webpage_url && (
              <a href={job.webpage_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-accent hover:underline">
            ...
            <div className="flex gap-3 pt-2">
              {user?.role === "JOB_SEEKER" && (
                ...
                >
                  <Send className="h-4 w-4" /> {applying ? "Applying…" : "Apply Now"}
                </Button>
              )}
              <Button variant="outline" onClick={() => navigate("/messages")} className="active:scale-[0.97] transition-transform">
                Message Employer
              </Button>
            </div>
          </CardContent>
