...
import { Camera, Briefcase, GraduationCap, Globe, Upload, FileText, Download, Loader2, Eye } from "lucide-react";
...
export default function Profile() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRecruiter) {
        await updateProfile({ companyName, description, phoneNumber, city, country, industry, logoUrl });
      } else {
        await updateProfile({
            ...
            <div className="flex flex-col items-center gap-3 sm:flex-row sm:items-start">
              <div className="relative shrink-0">
                {user?.profilePicture || user?.logoUrl ? (
                  <img src={user.profilePicture || user.logoUrl} alt="Avatar" className="h-20 w-20 rounded-full object-cover" />
                ) : (
        ...
        {isRecruiter ? (
          <ScrollReveal delay={120}>
            <Card className="glass">
              <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Briefcase className="h-4 w-4" /> Company Info</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1"><Label>Logo URL</Label><Input type="url" value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} /></div>
              </CardContent>
            </Card>
          </ScrollReveal>
        ) : (
          <>
            <ScrollReveal delay={120}>
              <Card className="glass">
                <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Briefcase className="h-4 w-4" /> Personal Info</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1"><Label>Bio</Label><Textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={2} /></div>
                </CardContent>
              </Card>
            </ScrollReveal>
            ...
            <ScrollReveal delay={160}>
              <Card className="glass">
                <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><GraduationCap className="h-4 w-4" /> Skills & Languages</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="space-y-1"><Label>Skills (comma-separated)</Label><Input value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="React, TypeScript, Node.js" /></div>
                  <div className="space-y-1"><Label>Languages (comma-separated)</Label><Input value={languages} onChange={(e) => setLanguages(e.target.value)} placeholder="English, Swedish" /></div>
                  <div className="space-y-1"><Label>Personal Statement</Label><Textarea value={personalStatement} onChange={(e) => setPersonalStatement(e.target.value)} rows={2} /></div>
                </CardContent>
              </Card>
            </ScrollReveal>
            ...
            <ScrollReveal delay={200}>
              <Card className="glass">
                <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Globe className="h-4 w-4" /> Links</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="space-y-1"><Label>Portfolio URL</Label><Input type="url" value={portfolioLink} onChange={(e) => setPortfolioLink(e.target.value)} placeholder="https://myportfolio.com" /></div>
                </CardContent>
              </Card>
            </ScrollReveal>
            ...
            <ScrollReveal delay={240}>
              <Card className="glass">
                <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><FileText className="h-4 w-4" /> CV / Resume</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {user?.cv && (
                      <>
                        ...
                        <Button type="button" variant="outline" size="sm" onClick={() => setCvPreviewOpen(true)}>
                          <Eye className="mr-1.5 h-4 w-4" /> Preview CV
                        </Button>
                      </>
                    )}
                  </div>
